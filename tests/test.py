...
try:
    ...
    acc_num = wait.until(lambda x: x.find_element_by_xpath('//h2[contains(text(),"New Account")]/..//span[text()="Account Number"]/../following-sibling::input[1]'))
    acc_num.send_keys(timestamp)
    rating_ddl = wait.until(lambda x: x.find_element_by_xpath('//h2[contains(text(),"New Account")]/..//span[text()="Rating"]/../following-sibling::div[1]//a'))
    ...
    save_btn.click()
    message = wait.until(lambda x: x.find_element_by_xpath('//div[contains(@class,"slds-theme--success")]/div/div/span[contains(@class,"toastMessage")]'))
    # print(message.text)
